...
async def raport_command(ctx):
    ...
    try:
        ...
        # Inicjalizuj system raportów z quota monitoring - NOWY!
        raport_system = RaportSystemWorkshop(
            api_key=YOUTUBE_API_KEY,
            quota_manager=quota_manager,  # PRZEKAŻ quota_manager!
            demo_mode=False  # PRAWDZIWE API CALLS - zużywa quota!
        )
        ...
        if result['success']:
            ...
            embed.add_field(
                name="🔄 **Następne kroki**",
                value="• Użyj `!name` aby analizować nazwiska\n• Sprawdź `!status` aby zobaczyć zużycie quota",
                inline=False
            )
...
async def status_command(ctx):
    ...
    try:
        ...
        try:
            ...
            sledz_system = SledzSystem()
            room_channels = sledz_system.get_room_channels(ctx.channel.name)
            total_channels = len(room_channels)
            
            embed.add_field(
                name="📺 **Kanały w pokoju**",
                value=f"```\nPokój: #{ctx.channel.name}\nKanały: {total_channels}\n```",
                inline=False
            )
        except Exception as e:
            embed.add_field(
...
async def sync_command(ctx, action=None):
    """
    Synchronizacja z Railway - eksport i backup danych
    """
    if not ctx.author.guild_permissions.administrator and ctx.author.id != 1395727079152029766:
        await ctx.send("❌ **BRAK UPRAWNIEŃ** - tylko administrator może używać tej komendy")
        return
    ...
    if action == 'export':
        try:
            ...
            report_file = "raw_data/raw_raport_2025-07-26_showbiz.json"
            ...
            if os.path.exists(report_file):
                ...
                embed = discord.Embed(
                    ...
                    description=f"Plik: `{report_file}`",
                    color=0x00ff00,
                    timestamp=datetime.now(timezone.utc)
                )
                ...
                embed.add_field(
                    name="📈 **Statystyki**",
                    value=f"```\nKanałów: {data.get('channels_processed', 0)}\nFilmów: {data.get('total_videos', 0)}\nQuota: {data.get('quota_used', 0)}\nData: {data.get('date', 'N/A')}\n```",
                    inline=False
                )
                
                # Pokaż pierwsze 3 filmy jako przykład
                videos = data.get('videos', [])
                if videos:
                    sample_videos = videos[:3]
                    video_list = "\n".join([
                        f"• {v.get('Title', 'N/A')[:50]}... ({v.get('Video_ID', 'N/A')})"
                        for v in sample_videos
                    ])
                    
                    embed.add_field(
                        name="📺 **Przykładowe filmy** (pierwsze 3)",
                        value=f"```\n{video_list}\n```",
                        inline=False
                    )
                ...
                await ctx.send("❌ **PLIK NIE ISTNIEJE** - uruchom `!raport` aby wygenerować dane")
                
        except Exception as e:
            await ctx.send(f"❌ **BŁĄD EKSPORTU**: {str(e)}")
        ...
        try:
            ...
            embed = discord.Embed(
                title="📊 **STATUS RAILWAY**",
                description="Sprawdzenie plików i konfiguracji",
                color=0x00ff00,
                timestamp=datetime.now(timezone.utc)
            )
            ...
            if os.path.exists('channels_config.json'):
                with open('channels_config.json', 'r') as f:
                    config = json.load(f)
                showbiz_channels = len(config.get('channels', {}).get('showbiz', []))
                files_status.append(f"✅ channels_config.json ({showbiz_channels} kanałów showbiz)")
            else:
                files_status.append("❌ channels_config.json")
            
            # quota_usage.json
            if os.path.exists('quota_usage.json'):
                files_status.append("✅ quota_usage.json")
            else:
                files_status.append("❌ quota_usage.json")
            
            # raw_data folder
            if os.path.exists('raw_data'):
                raw_files = len([f for f in os.listdir('raw_data') if f.endswith('.json')])
                files_status.append(f"✅ raw_data/ ({raw_files} plików)")
            else:
                files_status.append("❌ raw_data/")
        ...
        await ctx.send("🔄 **BACKUP** - funkcja w przygotowaniu")
    
    elif action == 'info':
        embed = discord.Embed(
            title="ℹ️ **INFORMACJE O RAILWAY**",
            description="Status deployment i konfiguracji",
            color=0x0099ff,
            timestamp=datetime.now(timezone.utc)
        )
        ...
        embed.add_field(
            name="🚀 **Deployment**",
            value="Railway.app - automatyczny deployment z GitHub",
            inline=False
        )
        
        embed.add_field(
            name="📊 **Dostępne akcje**",
            value="```\n!sync status - sprawdź pliki\n!sync export - eksport raportu\n!sync backup - backup danych\n!sync info - informacje\n```",
            inline=False
        )
        ...
        embed = discord.Embed(
            title="🔄 **SYNCHRONIZACJA RAILWAY**",
            description="Zarządzanie danymi na serwerze",
            color=0x0099ff,
            timestamp=datetime.now(timezone.utc)
        )
        ...
        embed.add_field(
            name="⚠️ **Uwaga**",
            value="Tylko administrator może używać tych komend",
            inline=False
        )
